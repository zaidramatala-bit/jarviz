...
Analysing `invokedynamic` can be tricky as the method handle can change dynamically at runtime. Each `invokedynamic` call site names a bootstrap method (for lambdas and method references this is `java.lang.invoke.LambdaMetafactory#metafactory` or `#altMetafactory`) which links the call site on first execution. The bootstrap method itself is not a dependency of the code being analysed, so Jarviz instead records the method handles passed as bootstrap method arguments, which point at the actual implementation (for example the synthetic `lambda$doThat$0` method generated for a lambda body, or the referenced method for `Helper::process`). Call sites that carry no method handle arguments (for example string concatenation via `java.lang.invoke.StringConcatFactory`) produce no coupling. It should be noted that static code analysis may not be accurate if the code is extensively using `invokedynamic` calls to change behavior at runtime.
